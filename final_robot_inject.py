...
new_robot_setup = """
    ...
    loader.load(url, (gltf) => {
      ...
      // Position the robot in the Hero section
...
if target_setup in content and "GLTF REALISTIC ROBOT" not in content:
    content = content.replace(target_setup, new_robot_setup + "\n" + target_setup)
...
target_render = "      // ── POST PROCESSING ───────────────────────────────────────────────────────"
if target_render in content and "GLTF REALISTIC ROBOT RENDER" not in content:
    content = content.replace(target_render, new_robot_render + "\n" + target_render)
...
# Append click listener to end of file
